...
class RssTicker extends HTMLElement {
  static get observedAttributes() {
    return ["rss","speed","maxitems","separator","dark","jsonproxy","debug"];
  }
  constructor() {
    super();
  ...
  async render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host { display:inline-block;height:32px;width:360px;max-width:640px;flex:0 1 360px;
    ...
    try {
      ...
      this.log("Items geladen:", items.length, {rss, jsonProxy});
  ...
  async loadItems(rssUrl, maxItems, jsonProxy) {
    if (jsonProxy) {
      this.log("Hole vor-konvertierten JSON-Feed via jsonProxy:", jsonProxy);
      const res = await fetch(jsonProxy, { cache: "no-store" });
      if (!res.ok) throw new Error(`Proxy-Feed: HTTP ${res.status}`);
    ...
    this.log("Hole RSS über rss2json:", api);
